...
- **[IN PROGRESS — wave 3] i18n sweep:** the website chrome batch is wired — header, footer and mobile menu now render from the `header`/`footer` catalogs (en/fr/nl kept in lockstep, new keys added for account/menu aria/footer link columns/tagline). Dashboard nav chrome wired in the follow-up batch: buyer module sidebar + seller/admin settings sidebars (and Log Out) resolve from new `buyer/seller/admin.nav` namespaces (en/fr/nl lockstep); legacy cp1252 mojibake in the catalogs (…/—/É/À) repaired. Dashboard home surfaces now fully wired (wave 3c): buyer.home (26 keys), seller.home (33) and admin.home (28) namespaces added in lockstep — welcome headings, stat cards, chart titles/configs/legends, table titles + column headers (hook-factory pattern), empty states and row actions all resolve per locale; seller analytics month labels now use `useLocale()`. Remaining: deep module detail surfaces (wallet flows, settings forms, modals).
...
| 20 | P2 | UX | i18n sweep, empty states, theme FOUC, dialog focus | PARTIAL (FOUC fixed; header/footer + dashboard navs + all three dashboard homes done; wave 3 2026-09-05 complete: buyer wallet/flows + settings, seller settings, admin settings, and the full admin list-table chrome across auctions/bids/buyers/sellers/tickets; shared tab-filter labels translated 2026-09-05 via `common.dataTable` model-as-keys with literal fallback (4fa1ff7); catalogs at 1545 keys ×3 locales; remaining raw strings are enum/status values (DB data, not chrome) and mock-data values — deliberate and documented) |
...
| 24 | P1 | Security | `sanitizeUser` spread the entity into a PLAIN object — `@Exclude` metadata does not survive that, so live pending `otpCode`/`otpExpiry` were serialized on the wire in every `/auth/*` response (login/register/oauth/reset, 8 call sites). `passwordHash` was rest-destructured out, so only the OTP pair leaked | RESOLVED (2026-09-08, G-wave verification): explicit public-whitelist projection + 2 regression specs; wire-verified raw login body contains no `otpCode`. **Convention:** never return entity-spreads from auth paths; use explicit projections |
