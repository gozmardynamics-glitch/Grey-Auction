...
function ShareIconButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={label}
      className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-muted-foreground transition-all hover:bg-primary/10 hover:text-primary hover:shadow-md"
    >
      <Icon />
    </button>
  );
...
export function ShareButtons({ url, title, description }: ShareButtonsProps) {
  ...
  const handleShare = (shareKey: string) => {
    const btn = shareButtons.find((b) => b.key === shareKey);
    if (!btn) return;
      ...
      <div className="sm:hidden">
        <Sheet>
          ...
          <SheetContent side="bottom" className="pb-8">
            <SheetHeader>
              <SheetTitle>Share this auction</SheetTitle>
            </SheetHeader>
