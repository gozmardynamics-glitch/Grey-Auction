...
export function TrendingLotsCards({
  ...
  return (
    <Card 
      className="overflow-hidden border border-border/50 transition-all duration-300 hover:shadow-lg hover:border-primary/20 hover:-translate-y-0.5 group flex flex-col cursor-pointer"
    ...
    >
      ...
      <CardContent className="flex-1 p-3 space-y-1">
        ...
        {auction.sellerName && (
          <p className="text-[10px] text-muted-foreground truncate">{auction.sellerName}</p>
        )}
      ...
      {/* Buttons — Smaller */}
      <CardFooter className="p-2.5 pt-0 w-full flex gap-1.5" onClick={(e) => e.stopPropagation()}>
        {auction.status === 'active' ? (
          <>
            <Button variant="outline" size="sm" className="flex-1 h-7 text-[10px] border-primary text-primary hover:bg-primary/10" onClick={() => onBidClick?.(auction.id)}>
              Buy Now
            </Button>
