...
export default function BuyerRegisterPage() {
  ...
  const form = useForm<BuyerRegisterFormValues>({
  ...
  const onSubmit = async (data: BuyerRegisterFormValues) => {
    setIsLoading(true);

    try {
      console.log('Register data:', data);
      await new Promise((resolve) => setTimeout(resolve, 1500));
      router.push('/auth/buyer/register/otp');
    } catch {
  ...
  const handleGoogleLogin = () => {
    signIn('google', { callbackUrl: '/auth/buyer/register/otp' });
  };
